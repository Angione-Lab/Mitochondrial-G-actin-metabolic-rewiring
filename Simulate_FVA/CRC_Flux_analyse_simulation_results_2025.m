...
load ("Results\tinit_high_glucose\CRC.mat");

[new_grRules,new_genes,new_rxnGeneMat] = translateGrRules(model.grRules, 'Symbol');
model.grRules = new_grRules;
model.genes = new_genes;
model.rxnGeneMat = new_rxnGeneMat;

tbl_maxFlux = rows2vars(readtable('Results/tinit_high_glucose/tinit_FVA_max_high_glucose.csv'));
maxFlux = cell2mat(table2cell(tbl_maxFlux(2:end, 2:end)));

tbl_minFlux = rows2vars(readtable('Results/tinit_high_glucose/tinit_FVA_min_high_glucose.csv'));
...
formulas = printRxnFormula(model, 'metNameFlag',true);
formulas = strrep(formulas, '[]', '');

backwardFormulas = formulas(revIDs);
for i = 1:length(backwardFormulas)
    rxn = backwardFormulas{i};
    if contains(rxn, '<=>')
        parts = strsplit(rxn, '<=>');
    elseif contains(rxn, '-->')
        parts = strsplit(rxn, '-->');
    else
        continue; % skip if no recognizable direction
    end
    % Trim spaces and reverse
    lhs = strtrim(parts{1});
    rhs = strtrim(parts{2});
    backwardFormulas{i} = [rhs, ' <=> ', lhs];
end

formulas = [formulas; backwardFormulas];
...
t1 = [{'Reaction ID', 'Reaction name', 'Formula', 'Pathway', 'GPR rules', 'APC_WT1','APC_WT2','APC_WT3', 'APC_m4_1','APC_m4_2','APC_m4_3',...
 'Average Maxflux wildtype','Average Maxflux m4', 'Foldchange m4_vs_wildtype', 'log2(Foldchange m4_vs_wildtype)'}; ...
...
xlswrite(['Results/high_glucose_results_2025'], t1, 'reactions');
%
...
upregulated = and(logfluxFoldChange_rxns > prctile(logfluxFoldChange_rxns, 95), logfluxFoldChange_rxns > 1.5);
downregulated = and(logfluxFoldChange_rxns <= prctile(logfluxFoldChange_rxns, 5), logfluxFoldChange_rxns < -0.25);
...
t1 = [{'Reactions', 'Reaction Names','pathways', 'FC M4 vs wildtype'}; ...
    cell(uniquerxns),cell(uniquerxnNames) cell(enriched_subsystems),num2cell(fluxFoldChange_rxns)];
xlswrite(['Results/high_glucose_reaction_foldchange_2025'], t1, 'reactions');
...
fig.PaperSize = [10, 17];
%print('-fillpage', 'Results/crc_reactions_low_glucose', '-dpdf');

%%
%compartmentRxns = [];
%for i = 1:length(model.comps)
%    disp(model.comps{i})
%    compartmentRxns = [compartmentRxns; getReactionCompartment(model, model.comps{i})];
%
%end
%
%T1 = cell2table([rxns, rxnNames, subSystems, formulas, num2cell(fluxFoldChange(:,1)), num2cell(fluxFoldChange(:,2))]);
%T2 = cell2table(compartmentRxns);
%
%T1.Properties.VariableNames ={'rxns','rxnNames', 'subSystems', 'formulas','FC_1',  'FC_2'};
%T2.Properties.VariableNames ={'rxns', 'comp'};
%T = innerjoin(T1, T2);
%
%fluxFoldChange = [T{:,'FC_1'},T{:,'FC_2'} ];
%subSystems = cellfun(@(x,y) [x  ' (' y ') '],T{:,"subSystems"},T{:,"comp"},'un',0) ;
%rxns = T{:,"rxns"};
%rxnNames = T{:,"rxnNames"};
...
xlswrite(['Results/high_glucose_results_2025'], t2, 'pathways');
...
xlswrite(['Results/high_glucose_results_2025'], enrichment_table, 'rxn_enrichment_table');
...
xlswrite(['Results/high_glucose_results_2025'], enrichment_table, 'enrichment_table');
...
xlswrite(['Results/high_glucose_results_2025'], t4, 'metabolites');
...
function compartmentRxns = getReactionCompartment(model, compartment)
    ...
    compartmentRxns=[compartmentRxns, cellstr(repmat(compartment,size(compartmentRxns,1),1))];
